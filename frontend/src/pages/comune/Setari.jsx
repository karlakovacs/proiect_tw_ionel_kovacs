import "../../css_files/comune/Setari.css";
...
const Setari = () => {
	...
	const validateParole = () => {
		...
		if (parolaNoua === parolaVeche) {
			errors.parolaNoua =
				"Noua parolă nu poate fi identică cu parola veche!";
		}
	...
	const handleSchimbareParola = async () => {
		...
		try {
			const response = await fetch(
				`${VITE_API_URL}/utilizatori/schimbare-parola/${id}`,
				{
					method: "PUT",
	...
	return (
		<div className="Setari">
			...
			{!urlSemnatura ? (
				<p style={{ color: "red" }}>
					Nu aveți o semnătură virtuală setată. Vă recomandăm să o
					adăugați.
				</p>
			) : (
				<img
			...
			<form>
				...
				<input
					...
					required
				/>
				{parolaErrors.parolaNoua && (
					<p style={{ color: "red" }}>{parolaErrors.parolaNoua}</p>
				)}
