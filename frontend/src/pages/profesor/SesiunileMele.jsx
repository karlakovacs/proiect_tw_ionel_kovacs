import "../../css_files/profesor/SesiunileMele.css";
		...
		const fetchSesiuni = async () => {
			try {
				const response = await fetch(
					`${VITE_API_URL}/sesiuni/profesor/${id}`
				);
				const data = await response.json();

				if (!response.ok) {
					throw new Error(
						data.message || "Eroare la încărcarea sesiunilor."
					);
				}
					...
					<tbody>
						{sesiuni.map((sesiune) => {
							...
							return (
								...
								>
									<td>
										{dataInceput.toLocaleDateString(
											"ro-RO"
										)}
									</td>
									<td>
										{dataSfarsit.toLocaleDateString(
											"ro-RO"
										)}
									</td>
